...
public class SerialPortManagerV2 : MonoBehaviour
{
    ...
    public void Connect()
    {
        ...
        try
        {
            ...
            {
                if (port == GameConstantsV2.SERIAL_PORT_NAME)
                {
                    portExists = true;
                    break;
                }
            ...
            if (!portExists)
            {
                Debug.LogWarning($"[SerialPortManager] Port {GameConstantsV2.SERIAL_PORT_NAME} not found. Available ports: {string.Join(", ", availablePorts)}");
                _isReconnecting = false;
                return;
            }
            
            // SerialPort 初期化
            _serialPort = new SerialPort(GameConstantsV2.SERIAL_PORT_NAME, GameConstantsV2.SERIAL_BAUD_RATE);
            _serialPort.ReadTimeout = SerialPort.InfiniteTimeout;  // ★ 変更：ブロッキング待機
            _serialPort.WriteTimeout = 100;
            _serialPort.Open();
            
            if (GameConstantsV2.DEBUG_MODE)
                Debug.Log($"[SerialPortManager] Connected to {GameConstantsV2.SERIAL_PORT_NAME} at {GameConstantsV2.SERIAL_BAUD_RATE} baud");
        }
        catch (Exception ex)
        {
